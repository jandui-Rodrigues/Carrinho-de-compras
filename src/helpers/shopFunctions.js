 ...
 */
export const createCartProductElement = ({ id, price, pictures, title }) => {
  const li = document.createElement('li');
  li.className = 'cart__product';
  const infoContainer = createCustomElement('div', 'conteiner-div');

  const img = createProductImageElement(pictures[0].url);

  const imgContainer = createCustomElement('div', 'cart__product__info-container');
  imgContainer.appendChild(img);
  const TitleProduct = title.split(' ').splice(0, 2).join(' ');
  const span = createCustomElement('span', 'opa', TitleProduct);
